...
function makeReplyObj(delete_password, threadDate) {
  ...
  return ({ text, delete_password, reported: false, created_on, updated_on: created_on});
}
...
function makeThreadObj(board, delete_password, numReplies) {
  ...
  return ({ board, text, replies, delete_password, reported: false, created_on, bumped_on });
}
